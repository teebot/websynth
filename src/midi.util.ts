 ...
 */
export const midiInputs$ = midiAccess$.pipe(
  map((midi: MIDIAccess) => {
    return Array.from(midi.inputs).map(([id, input]) => input);
  })
);

export const midiInputTriggers$: Observable<any> = midiInputs$.pipe(
  mergeMap((inputs: any[]) =>
    Observable.create(observer =>
      inputs.forEach(i => (i.onmidimessage = event => observer.next(event)))
    )
  ),
  map((m: MIDIMessageEvent) => {
    const [origin, key, velocity] = Array.from(m.data);
    const keyP =
      origin >= 144 && origin <= 159 && velocity > 0
        ? keyPressed.On
        : keyPressed.Off;
